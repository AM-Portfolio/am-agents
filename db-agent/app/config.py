...
class Settings(BaseSettings):
    ...
    APP_PORT: int = Field(default=8140)
    ...
    DB_AGENT_READ_ONLY_DEFAULT: bool = Field(default=True)
    DB_AGENT_ALLOW_WRITES: bool = Field(default=False)
    DB_AGENT_MAX_ROWS: int = Field(default=100)
    DB_AGENT_TIMEOUT_SECONDS: int = Field(default=30)
    DB_AGENT_INTENT_MIN_CONFIDENCE: float = Field(default=0.75, ge=0.0, le=1.0)
    DB_AGENT_REQUIRE_BACKEND_FOR_AGENTS: bool = Field(default=True)
    ...
    GCP_MCP_ENDPOINTS_JSON: str = Field(default="{}")
    ...
    GRAFANA_SERVICE_ACCOUNT_TOKEN: str | None = Field(default=None)
    INFLUX_URL: str | None = Field(default=None)
    INFLUX_TOKEN: str | None = Field(default=None)

...
def _peek_messages_via_ui_sync(topic: str, *, limit: int, from_tail: bool) -> dict[str, Any]:
    ...
    with httpx.Client(timeout=settings.DB_AGENT_TIMEOUT_SECONDS, follow_redirects=True) as client:
...
def _peek_messages_sync(topic: str, *, limit: int, from_tail: bool = False) -> dict[str, Any]:
    ...
    group_id = f"db-agent-peek-{uuid.uuid4().hex[:12]}"
...
class GrafanaAdapter:
    ...
    def available(self) -> bool:
        return bool(
            ...
            and (settings.GRAFANA_API_KEY or settings.GRAFANA_SERVICE_ACCOUNT_TOKEN)
        )
    ...
    async def execute(
        ...
        raise RuntimeError(
            "Grafana adapter not implemented — enable MCP satellite (mcp-grafana)"
        )
...
class InfluxAdapter:
    ...
    async def execute(
        ...
        raise RuntimeError(
            "Influx adapter not implemented — use Grafana MCP query_influxdb"
        )
...
class LokiAdapter:
    ...
    async def execute(
        ...
        raise RuntimeError(
            "Loki adapter not implemented — enable MCP satellite (mcp-grafana with loki)"
        )
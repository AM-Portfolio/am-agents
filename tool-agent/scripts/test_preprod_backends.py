#!/usr/bin/env python3
"""Preprod integration: health, MCP bridge, query redis/postgres/vault."""
...
QUERY_CASES = [
    ...
    ("query-redis-scan", {"query": "list redis session keys", "backend": "redis", "read_only": True}),
    (
    ...
    (
        ...
        {"query": "how many portfolios in postgres", "backend": "postgres", "read_only": True},
    ),
    (
        "query-vault-list",
        {"query": "list vault secrets under apps/data/preprod/infra", "backend": "vault", "read_only": True},
    ),
    (
        "query-vault-read",
...
def main() -> int:
    passed = failed = 0
    print(f"Preprod tool-agent tests -> {BASE}\n")

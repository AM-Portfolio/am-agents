...
from tools.qdrant import safety
from tools.qdrant.adapter import QdrantAdapter
from tools.qdrant.search import parse_rules, resolve


class QdrantTool(BaseIntegrationTool):
...
def get_tool() -> QdrantTool:
    return QdrantTool(Path(__file__).resolve().parent)

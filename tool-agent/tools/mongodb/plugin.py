...
from tools.mongodb import adapter, safety
from tools.mongodb.search import parse_rules, resolve


class MongodbTool(BaseIntegrationTool):
    def __init__(self, tool_dir: Path) -> None:
        super().__init__(tool_dir)
        self._adapter = adapter.MongoAdapter()
...
def get_tool() -> MongodbTool:
    return MongodbTool(Path(__file__).resolve().parent)

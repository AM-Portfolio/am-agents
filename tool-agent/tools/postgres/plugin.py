...
from tools.postgres import adapter, safety
from tools.postgres.search import parse_rules, resolve


class PostgresTool(BaseIntegrationTool):
    def __init__(self, tool_dir: Path) -> None:
        super().__init__(tool_dir)
        self._adapter = adapter.PostgresAdapter()
...
def get_tool() -> PostgresTool:
    return PostgresTool(Path(__file__).resolve().parent)

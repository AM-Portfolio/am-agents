...
from tools.kafka import safety
from tools.kafka.adapter import KafkaAdapter
from tools.kafka.search import parse_rules, resolve


class KafkaTool(BaseIntegrationTool):
    def __init__(self, tool_dir: Path) -> None:
        super().__init__(tool_dir)
        self._adapter = KafkaAdapter()
...
def get_tool() -> KafkaTool:
    return KafkaTool(Path(__file__).resolve().parent)

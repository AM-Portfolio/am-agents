...
from tools.temporal import safety
from tools.temporal.adapter import TemporalAdapter
from tools.temporal.search import parse_rules, resolve


class TemporalTool(BaseIntegrationTool):
    def __init__(self, tool_dir: Path) -> None:
        super().__init__(tool_dir)
        self._adapter = TemporalAdapter()
...
def get_tool() -> TemporalTool:
    return TemporalTool(Path(__file__).resolve().parent)

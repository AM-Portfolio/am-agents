...
from app.models.intent import IntentDocument, ParseSource, ToolsQueryRequest
...
def check_agent_requirements(
    ...
    *,
    ...
    if not is_agent_caller(agent_caller):
        return None
    if settings.TOOL_AGENT_REQUIRE_BACKEND_FOR_AGENTS and not request.backend:
...
def check_confidence(
    ...
    *,
    ...
    if not is_agent_caller(agent_caller):
        return None
    if intent.confidence < settings.TOOL_AGENT_INTENT_MIN_CONFIDENCE:
        return (
            f"Intent confidence {intent.confidence:.2f} below minimum "
            f"{settings.TOOL_AGENT_INTENT_MIN_CONFIDENCE:.2f}"
        )
    return None
